...
export class ComputeWorkerBuilder {
  ...
  private buffers: Map<string, BufferDefinition> = new Map();
  ...
  addStaging(name: string, data: BufferData): this {
    ...
    const { buffer, readbackBuffer } = createStagingBuffer(
      this.device,
      data,
      `Staging: ${name}`
    );
   ...
   */
  addPass<T extends typeof ComputeShader>(
    ShaderClass: T,
    ...
    for (const bindingName of bindings) {
      if (!this.buffers.has(bindingName)) {
        throw new Error(
          `Cannot add pass: buffer "${bindingName}" has not been defined yet. ` +
    ...
    const shader = new (ShaderClass as any)() as InstanceType<T>;
  ...
  build(): ComputeWorkerInstance {
    ...
    return new ComputeWorkerInstance(
      this.device,
      this.buffers,
      this.passes,
      this.isOneShot
   ...
   */
  private compilePass(pass: PassDefinition): void {
    const shader = pass.shader as ComputeShader;
    ...
    for (let i = 0; i < pass.bindings.length; i++) {
      ...
      if (!bufferDef) {
        throw new Error(
          `Buffer "${bindingName}" not found when creating bind group`
        );
      }
