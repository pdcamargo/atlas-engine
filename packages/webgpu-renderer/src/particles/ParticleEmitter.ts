...
export class ParticleEmitter extends SceneNode {
  ...
  private gpuConfig: GPUEmitterConfig;

  // Emission control
  public enabled: boolean;
  private emissionTime: number = 0;
  ...
  async initialize(device: GPUDevice, format: GPUTextureFormat): Promise<void> {
    ...
    this.particleSystem = new ParticleSystem(
      ...
      this.config.texture,
      this.config.blendMode ?? ("normal" as ParticleBlendMode)
    );
  ...
  update(deltaTime: number): void {
    if (!this.particleSystem || !this.initialized) return;
   ...
   */
  private buildGPUConfig(config: ParticleEmitterConfig): GPUEmitterConfig {
    // Normalize ranges
    const lifetime = normalizeRange(
      config.lifetime ?? { min: 1.0, max: 2.0 }
    );

    const velocity = normalizeVector3Range(
      config.velocity ?? { x: 0, y: 1, z: 0 }
    );
    ...
    // Handle color config
    const defaultStartColor: [number, number, number, number] = [1, 1, 1, 1];
    const defaultEndColor: [number, number, number, number] = [1, 1, 1, 0];
    ...
    // Handle rotation
    const rotationSpeed = normalizeRange(
