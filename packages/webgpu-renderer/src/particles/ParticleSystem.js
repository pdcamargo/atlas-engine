...
export class ParticleSystem {
    ...
    totalTime = 0;
    constructor(device, format, maxParticles, texture, blendMode = "normal") {
        this.device = device;
        this.format = format;
    ...
    async initialize() {
        ...
        if (!this.texture) {
            ...
            this.device.queue.writeTexture({ texture: this.defaultTexture }, whitePixel, { bytesPerRow: 4 }, { width: 1, height: 1 });
        }
        // Create sampler
     ...
     */
    updateEmitterConfig(config) {
        if (!this.emitterConfigBuffer)
            return;
     ...
     */
    update(deltaTime, emissionRate, enabled) {
        if (!this.updatePipeline || !this.emitPipeline)
            return;
