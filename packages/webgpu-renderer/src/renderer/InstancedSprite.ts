...
export class InstancedSprite extends Sprite {
  ...
  private _cachedWorldPosition: Vector3 = new Vector3();
  private _cachedWorldScale: Vector3 = new Vector3(1, 1, 1);
  private _worldCacheDirty: boolean = true;
  ...
  override addChild(): void {
    throw new Error(
      "InstancedSprite does not support scene graph hierarchy. Use regular Sprite if you need children."
    );
  }
   ...
   */
  override updateWorldMatrix(parentWorldMatrix?: Mat4): void {
    if (parentWorldMatrix) {
      throw new Error(
        "InstancedSprite cannot have a parent. Use regular Sprite if you need hierarchy."
      );
    }
   ...
   */
  override setScale(scale: any): void {
