import { sys, AssetServer, LoadState, QueryBuilder, } from "@atlas/core";
...
export const tileSetLoadingSystem = sys(({ commands }) => {
    ...
    for (const [entity, tileMap] of commands.query(unsynced).all()) {
        ...
        for (const tileSet of allTileSets) {
            ...
            const imageAsset = assetServer.getAsset(handle);
            if (!imageAsset || !imageAsset.image) {
                console.warn(`[TileSetLoading] ImageAsset loaded but has no image data`);
                allTileSetsLoaded = false;
                continue;
            }
            ...
            if (syncedGrids > 0) {
                console.log(`[TileSetLoading] Created ${syncedGrids} pending tile grid(s) for tileset ${tileSet.id}`);
            }
            // Sync any pending tiles that were waiting for this texture
            const syncedCount = tileMap.syncPendingTiles();
            if (syncedCount > 0) {
                console.log(`[TileSetLoading] Synced ${syncedCount} pending tiles for tileset ${tileSet.id}`);
            }
            tileMap.markDirty();
        }
