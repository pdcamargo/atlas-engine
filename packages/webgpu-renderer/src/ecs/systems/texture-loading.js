...
const unsyncedAnimatedSprites = new QueryBuilder(AnimatedSprite).without(TextureSynced);
/**
...
export const textureLoadingSystem = sys(({ commands }) => {
    ...
    for (const [entity, sprite] of [...animated, ...normal]) {
        ...
        const needsSync = isAnimated && sprite._needsTextureSync;
        ...
        if (isAnimated) {
            sprite._needsTextureSync = false;
        }
